...
if [ $AWS_ACCESS_KEY_ID ]; then
  ...
  if [ $1 -eq 0 ]; then
    ...
    if ! docker-machine inspect $NAME &> /dev/null; then
      ...
      docker $(docker-machine config $NAME) run -d -p 172.17.0.1:53:53 -p 172.17.0.1:53:53/udp -p 8500:8500 --name $NAME-consul --net tldr-overlay $REGISTRY/consul -server -bootstrap-expect 1
    else
